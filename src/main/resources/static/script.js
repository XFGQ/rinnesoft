...
function renderProjects(filter) {
  ...
  if (zorbo && (filter === "all" || filter === "team")) {
    let iconName = zorbo.techIcon ? zorbo.techIcon.toLowerCase() : "";
    let iconSrc = `https://cdn.simpleicons.org/${iconName}/white`;

    // Java için klasik fincan logosunu çek
    if (iconName === "java") {
      iconSrc =
          ...
          <div class="project-card featured" data-category="${zorbo.category}">
              <div class="featured-header">
                  <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                      ...
                      ${zorbo.techIcon ? `<img src="${iconSrc}" class="tech-logo" style="width: 32px; height: 32px; margin: 0;" alt="${zorbo.techIcon}" onerror="this.outerHTML='<span class=\\'fallback-icon\\'>' + this.alt + '</span>'">` : ""}
                  </div>
  ...
  filtered.forEach((p) => {
    ...
    // Diğer projeler için de aynı Logo/Java kontrolünü ekliyoruz
    let iconName = p.techIcon ? p.techIcon.toLowerCase() : "";
    let iconSrc = `https://cdn.simpleicons.org/${iconName}/white`;
    ...
    card.innerHTML = `
            <div class="project-card-header" style="flex-wrap: wrap; gap: 10px;">
                <div>
                    <span class="project-badge ${badgeClass}">${badgeText}</span>
                    <h3 style="word-break: break-word; margin-bottom: 0;">${p.title}</h3>
                </div>
                ${p.techIcon ? `<img src="${iconSrc}" class="tech-logo" style="width: 30px; height: 30px; margin: 0;" alt="${p.techIcon}" onerror="this.outerHTML='<span class=\\'fallback-icon\\'>' + this.alt + '</span>'">` : ""}
            </div>
